...
## under TRANSFORM, for gdal functions 
...
def vrt_mosaic(in_dir, out_vrt):
    """uses gdal to create virtual (vrt) mosaic--'out_vrt_name'-- from rasters in 'in_dir', input directory"""
    file_list = [file for file in os.listdir(in_dir) if os.path.splitext(file)[-1] == '.tif'] 
...
def create_mask(rast_arr, thresh, remove):
    ...
    if remove == "lte":
        masked_arr =np.where(arr_cp > thresh, 1, 0)
    elif remove == "gte":
        masked_arr=np.where(arr_cp < thresh, 1, 0)
    return mask_arr
...
def plot_images(in_dir, sensor_string, out_dir):
    ...
    @interact
    def show_images(file=[f for f in sorted(os.listdir(out_dir)) if str(band) in f]):
...
def terrain_ruggedness_index(dem_fname, out_fname):
    """create Terrain Ruggedness Index (TRI) and Rugedness from DEM"""
    TRI_rast = DEMProcessing(destName=out_fname, srcDS=dem_fname, processing="TRI")
    return TRI_rast
...
def minimum_noise_fraction(inRast, numComponents=5, BrightnessNormalization=False): 
